...
idx_frame = [20, 24, 28, 32, 36]; % which pattern number do you want to display the responses to? You can try all, the pattern shape will be displayed by the code.
...
for id_frame = 1:numel(multi_begin_time)
    ...
    chosen_pattern = multi_frames(id_frame, :)
...
for id_frame = 1:numel(single_begin_time)
    ...
    stim_repeats = stim_repeats(1:min(n_reps, length(stim_repeats)));
    chosen_pattern = single_frames(id_frame, :)
