function [trace_chunked, stim_chunked] = extractEulerChunks(psth, samplingPSTH, stim, samplingStim)
...
trace_chunked = [psth(:, round(onStart*samplingPSTH ): round(onEnd*samplingPSTH)), psth(:, round(offStart*samplingPSTH) : round(offEnd*samplingPSTH)), psth(:, round(midStart*samplingPSTH) : round(midEnd*samplingPSTH))];
