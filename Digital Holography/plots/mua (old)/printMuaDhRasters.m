...
for dh_session = 3:6
    ...
    % Do zeros
    load(repetitions_file, 'zero_begin_time');
    for i_pattern = 1:numel(zero_begin_time)

        plotMEA()
        plotGridMEA()
        fullScreen()

        plotRasterMEA(SpikeTimes, zero_begin_time{i_pattern}, evt_binsize, evt_spacing, mea_channels, mea_map, mea_rate, n_max_rep);

        raster_name = ['dh' num2str(dh_session) '_mua_raster_zero' num2str(i_pattern)];
    ...
    for i_pattern = 1:numel(single_begin_time)
        ...
        plotRasterMEA(SpikeTimes, single_begin_time{i_pattern}, evt_binsize, evt_spacing, mea_channels, mea_map, mea_rate, n_max_rep);
