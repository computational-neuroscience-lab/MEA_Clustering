...
features_list_models = {};
features_list_neurons = {};
...
for i_exp = 1:numel(experiments)
    ...
    features_list_neurons{i_exp} = [norm_psth_neurons, norm_tsta, norm_areas];
    features_list_models{i_exp} = [norm_psth_models, norm_tsta, norm_areas];
        
    disp('')
end

createDataset(rgc_dataset_label, experiments, indices_list, features_list_neurons);
psths = psths_neurons;
...
createDataset(cnn_dataset_label, experiments, indices_list, features_list_models);
psths = psths_models;
save(getDatasetMat(), 'experiments', 'temporalSTAs', 'spatialSTAs', 'stas', 'psths', 'params', '-append')
load(getDatasetMat(), "cellsTable");
